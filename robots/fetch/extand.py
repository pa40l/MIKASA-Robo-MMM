...
from robots.fetch.base_yaw import (
    HeldBodies,
    arc_pull_cmd,
    jammed_turn,
    new_contacts,
...
from robots.fetch.stepping import (
    StepGuard,
    draw_rank,
    path_is_a_plan,
    pose_error,
...
class PandaArmSolverV2(PandaArmMotionPlanningSolver):
    def __init__(
        self,
        env: BaseEnv,
        debug: bool = False,
...
class PandaArmSapienSolver(PandaArmSolverV2):
    def __init__(
        self,
        env: BaseEnv,
        debug: bool = False,
...
class FetchMotionPlanningSapienSolver(PandaArmSapienSolver):
    ACTION_DIM = 13
    ...
    def __init__(self, *args, max_refine_steps: int | None = None, **kwargs):
        ...
        # (PandaArmSolverV2.__init__), which the property below routes
        # to the guard, so the guard has to exist first.
        self._guard = StepGuard(env)
        super().__init__(*args, **kwargs)
        shape = self.base_env.single_action_space.shape
        if shape != (self.ACTION_DIM,):
            raise ValueError(
                f"Fetch solver requires {self.ACTION_DIM}D actions, got {shape}"
            )
    ...
    def _report(self, stage: str, *, to_trace: bool = True, **fields) -> None:
        ...
        oracle's `say()`: the solver must not import from `planners.oracle`.
    ...
    def move_base_x_and_manipulation(self, target_tcp_pose, n_init_qpos=20):
        ...
        self.follow_forward_path_w_refinement(result)
        self.planner.update_from_simulation()
    ...
    def _slerp(q0, q1, t: float) -> np.ndarray:
        ...
        A line-for-line copy of `planners.oracle.oracle_common.slerp`, which is
        what `move_via` places its legs with. It is copied rather than imported because
        the solver must not import from `planners.oracle` (see `_report`), and it
    ...
    def _line_to_ik_goals(self, target_tcp_pose, goal_qpos, cur, cur_f, ref_yaw,
        ...
        """
        from robots.fetch.utils import goal_order, unwrap_toward
        p = self.planner
